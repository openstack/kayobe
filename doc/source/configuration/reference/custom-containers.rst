...
functionality that runs on the seed, infrastructure VMs, overcloud and Ansible
control hosts.
...
This is possible since the docker registry deployment step precedes the custom
container deployment step.

It is also possible to deploy a custom containerised docker registry as a custom
seed container. In this case, basic authentication login attempts can be
disabled by setting:

.. code-block:: yaml
  :caption: ``seed.yml``

  seed_manage_containers_registry_attempt_login: false

.. code-block:: yaml
  :caption: ``infra-vms.yml``

  infra_vm_manage_containers_registry_attempt_login: false

.. code-block:: yaml
  :caption: ``controllers.yml``

  controller_manage_containers_registry_attempt_login: false

.. code-block:: yaml
  :caption: ``storage.yml``

  storage_manage_containers_registry_attempt_login: false

.. code-block:: yaml
  :caption: ``monitoring.yml``

  monitoring_manage_containers_registry_attempt_login: false

.. code-block:: yaml
  :caption: ``ansible-control.yml``

  ansible_control_manage_containers_registry_attempt_login: false
